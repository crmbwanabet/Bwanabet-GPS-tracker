...
/**
 * Local SQLite database for offline location buffering.
 *
 * This is the ONLY database in the app. Points flow:
 *   GPS tick → insertLocation() → getPendingBatch() → Supabase sync → deleteSynced()
 *
 * Design decisions:
 * - Raw SQLite instead of Room to avoid kapt/annotation processing overhead
 *   and because the schema is simple (single table, no relations).
 * - INSERT OR IGNORE with unique index on (device_id, recorded_at) prevents dupes.
 * - Eviction only runs when count exceeds cap, not on every insert.
 * - All public methods are synchronized to prevent concurrent access issues
 *   between the location callback thread and the sync executor thread.
 */
class LocationDatabase private constructor(context: Context) : SQLiteOpenHelper(
    ...
    /**
     * Insert a location point. Skips silently on duplicate (same device+timestamp).
     * Evicts oldest points only when buffer exceeds MAX_PENDING.
     */
    @Synchronized
    fun insertLocation(point: JSONObject) {
        try {
            ...
            // Only run eviction when we've likely exceeded the cap
            // (check is cheap, delete is expensive — avoid on every insert)
    ...
    /**
     * Get a batch of the oldest pending points for sync.
     * Returns list of (row_id, json_payload) pairs.
     */
    @Synchronized
    ...
    /**
     * Delete specific rows by ID after successful sync.
     */
    @Synchronized
    fun deleteSynced(ids: List<Long>) {
        ...
        try {
            val db = writableDatabase
            // SQLite has a limit on variables in IN clause (~999).
            // 20 batch size is well under that, but chunk defensively.
    ...
    /**
     * Count of pending (unsynced) locations. Safe to call from any thread.
     */
    @Synchronized
    ...
    /** Internal count without synchronization (called from within synchronized methods). */
